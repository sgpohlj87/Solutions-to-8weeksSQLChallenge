...
FROM foodie_fi.subscriptions
WHERE plan_id IN ('1','2','3')
GROUP BY customer_id, start_date,plan_id, pro_annual_date
),
transform_data AS (
SELECT customer_id, SUM(basic_monthly) AS basic_monthly, SUM(pro_monthly) AS pro_monthly, SUM(pro_annual) AS pro_annual, pro_annual_date
FROM flag_data
GROUP BY customer_id, pro_annual_date
ORDER BY customer_id)
...
CASE WHEN basic_monthly=1 AND pro_annual=1 THEN 1 
          WHEN pro_monthly=1 AND pro_annual=1 THEN 1 
          ELSE NULL END AS upgrade
FROM transform_data
